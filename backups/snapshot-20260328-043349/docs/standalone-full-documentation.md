...
- **Integration**: MQTT-first with local provisioning web portal
...
## Getting Started (First Boot Provisioning)

Use this when flashing a fresh device or after factory reset.

1. Flash firmware and power up the device.
2. Connect phone/PC to the setup AP:
   - SSID: `{kProvisionApSsidPrefix}-XXXX`
   - Password: `ppcsetup` (or your configured `kProvisionApPassword`)
3. Open browser to `http://192.168.4.1`.
4. Enter Wi-Fi, MQTT broker settings, and customer/device IDs.
5. Optionally paste/upload CA certificate for TLS (`8883`).
6. Click **Save and Reboot**.
7. Device reboots and starts normal MQTT operation.

If provisioning is missing/invalid, device returns to this setup AP mode automatically.
...
- Default firmware now ships with empty Wi-Fi and MQTT endpoint credentials.
- Actual operational network settings are provisioned through local AP web setup and stored in NVS.
- MQTT plain port (`1883`) and TLS port (`8883`) are configurable in provisioning.
- Optional TLS assets:
  - broker CA certificate (provisioned and stored in NVS)
  - `kMqttClientCert`
  - `kMqttClientKey`
- Provisioning AP defaults:
  - `kProvisionApSsidPrefix`
  - `kProvisionApPassword`
...
- provisioning config (Wi-Fi credentials, broker host/ports/TLS/user/pass, CA cert, customer/device IDs)
...
### 10.9 Factory Reset (erase app NVS and reboot)

Safe reset command (requires confirmation):

```json
{ "request_id": "fr1", "cmd": "factory_reset", "confirm": "ERASE_NVS" }
```
...
If confirmation is missing or invalid, ACK message is:

- `factory_reset_confirmation_required`
...
Factory reset ACK messages:

- success: `factory_reset_restarting`
- failure: `factory_reset_failed`
...
## 20. Provisioning Portal and Factory Reset

### 20.1 When Provisioning Mode Starts

On boot, firmware checks NVS provisioning config.  
If required fields are missing/invalid, normal operation is skipped and local setup AP is started.

Required fields:

- Wi-Fi SSID
- MQTT host
- MQTT plain and TLS ports
- `customer_id`
- `device_id`

### 20.2 Provisioning Steps

1. Power device with empty/invalid provisioning config.
2. Connect phone/PC to AP:
   - SSID: `{kProvisionApSsidPrefix}-XXXX`
   - Password: `kProvisionApPassword` (or open AP if empty)
3. Open browser to:
   - `http://192.168.4.1`
4. Fill:
   - Wi-Fi SSID/password
   - MQTT host/ports/TLS/user/pass
   - CA certificate (paste or upload file)
   - `customer_id` and `device_id`
5. Click **Save and Reboot**.
6. Device stores config in NVS and reboots to normal MQTT mode.

### 20.3 Factory Reset Options

Option A (recommended, remote over MQTT):
...
- clears all app NVS data (provisioning + persisted runtime config/calibration/counter)
- reboots
- returns to provisioning AP mode on next boot

Option B (serial recovery with esptool, no MQTT needed):

- erase NVS partition only:
  - `esptool --chip esp32c5 --port COM30 erase-region 0x9000 0x5000`
- full chip erase:
  - `esptool --chip esp32c5 --port COM30 erase-flash`
...
## 22. Troubleshooting Quick Checklist
