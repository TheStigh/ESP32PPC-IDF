...
void VL53L1X::setup() {
  ...
  // Configure and initialize the ULD transport backend for this target framework.
  vl53l1x_configure_i2c(this->sda_pin_, this->scl_pin_, this->i2c_frequency_);
  if (i2c_init() != VL53L1_ERROR_NONE) {
    ESP_LOGE(TAG, "Failed to initialize I2C backend on SDA=%u SCL=%u at %u Hz",
...
VL53L1_Error VL53L1X::wait_for_boot() {
  ...
  while ((esphome::millis() - start) < this->timeout) {
